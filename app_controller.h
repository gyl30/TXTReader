...
class app_controller : public QObject
{
   ...
   private slots:
    ...
    void on_search_triggered(const QString& keyword);
    void on_find_next_result_triggered();
    void on_find_previous_result_triggered();

    void on_parsing_finished(size_t total_chapters);
    void on_chapter_content_ready(int chapter_index, const QString& content);
    void on_search_finished(const QList<int>& result_chapter_indices);

   private:
    void setup_connections();
    void load_new_file(const QString& file_path, const QString& regex);
    void load_chapter(int chapter_index);
    void navigate_to_search_result(int result_index);
    ...
    bool highlight_on_load_ = false;
    int target_match_index_in_chapter_ = -1;
    QString current_search_keyword_;
};
